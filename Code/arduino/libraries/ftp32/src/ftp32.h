...
#define FTP32_LOG 5             // TO SET DESIRED LOG_LVL
...
public:
  ...
  uint16_t connectWithPassword(const char* username, const char* password) {
    ...
    if(!_cClient.connect(_address, _port, _ctrl_timeout_us/1e3) 
      || !_isPositiveCompletion(_readResponse())
 ...
 /** @brief write data to the prevoulsy initiated upload transaction
  ...
  uint16_t finishUpload(){
    ...
    uint16_t response = _readResponse();
    return _isPositiveCompletion(response) ? 0 : _r_code;
  }
  ...
  uint16_t downloadSingleshot(const char* filename, String& dest){
    ...
    return _isPositiveCompletion(_readResponse()) ? 0 : _r_code;
  }
...
private:
  ...
  uint16_t _sendCmd(const char* cmd, const char* arg, uint16_t expectedResponseCode){
    ...
    uint16_t response = _readResponse();
    if( response == expectedResponseCode || 
        (_isPositiveCompletion(expectedResponseCode) && _isPositiveCompletion(response)) ||
        (_isPositiveIntermediate(expectedResponseCode) && _isPositiveIntermediate(response)) ||
        (_isPositivePreliminary(expectedResponseCode) && _isPositivePreliminary(response)) ){
      return 0;
    } else {
...
private:
  ...
  // Helper functions for FTP status code checking
  /** @brief Check if response code is a positive preliminary reply (1xx) */
  bool _isPositivePreliminary(uint16_t code) { return code >= 100 && code < 200; }
  
  /** @brief Check if response code is a positive completion reply (2xx) */
  bool _isPositiveCompletion(uint16_t code) { return code >= 200 && code < 300; }
  
  /** @brief Check if response code is a positive intermediate reply (3xx) */
  bool _isPositiveIntermediate(uint16_t code) { return code >= 300 && code < 400; }
  
  /** @brief Check if response code is a transient negative reply (4xx) */
  bool _isTransientNegative(uint16_t code) { return code >= 400 && code < 500; }
  
  /** @brief Check if response code is a permanent negative reply (5xx) */
  bool _isPermanentNegative(uint16_t code) { return code >= 500 && code < 600; }
  
  /** @brief Check if response code is a protected reply (6xx) */
  bool _isProtectedReply(uint16_t code) { return code >= 600 && code < 700; }
  
  /** @brief Check if response code indicates success (1xx, 2xx, 3xx) */
  bool _isSuccessful(uint16_t code) { return code >= 100 && code < 400; }
  
  /** @brief Check if response code indicates error (4xx, 5xx) */
  bool _isError(uint16_t code) { return code >= 400 && code < 600; }
};
